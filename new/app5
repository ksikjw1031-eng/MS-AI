# app.py
# --- Streamlit MVP: 한국어 뉴스 기반 PEST & SWOT (회사 선택 없이도 검색) ---
# .env 예시:
#   NEWSAPI_KEY=...
...
# ✅ 회사 선택(“선택 안 함” 지원) + 기타(직접입력)
with colA:
    company_choice = st.selectbox(
        "회사 선택",
        ["선택 안 함 (기술·도메인만)", "삼성SDS", "LG CNS", "SK C&C", "현대오토에버",
         "카카오엔터프라이즈", "네이버클라우드", "KT DS", "기타(직접입력)"],
        index=1,  # 기본: 삼성SDS
    )
    if company_choice == "기타(직접입력)":
        company = st.text_input("회사명 직접 입력", value="", placeholder="예: 한화시스템, NHN, 신세계I&C 등")
...
with colB:
    techs = st.multiselect(
        "관심 기술",
        ["AI", "RAG", "LangGraph", "Azure OpenAI", "Process Mining", "Cloud Native",
         "Azure AI Search", "MLOps", "Data Fabric"],
        default=["AI"]
    )
with colC:
...
# 자동 추천 쿼리 (회사 비어 있으면 자동 제외)
suggested_query = " ".join(
    [s for s in [company or "", " ".join(techs), " ".join(domains)] if s]
).strip()

st.text_input("뉴스 검색어", key="q_input", value=suggested_query, placeholder="예: AI 금융 / 삼성SDS AI 금융")
...
with opt_row1[2]:
    strict_and = st.checkbox("모든 키워드 반드시 포함(AND)", value=False, help="체크 시 '단어1 AND 단어2 AND ...' 형태로 검색합니다.")
...
def fetch_news_ko(query: str, cnt: int, freshness: str, use_and: bool = False):
    ...
    q_ready = query.strip()
    if not q_ready:
        raise RuntimeError("검색어가 비어 있습니다. 회사/기술/도메인에서 최소 1개는 선택 또는 입력하세요.")

    # ✅ AND 검색 옵션
    if use_and:
        terms = [t for t in q_ready.split() if t]
        q_ready = " AND ".join(terms) if terms else q_ready
...
def build_messages(company: str, techs: list[str], domains: list[str], news: list[dict]):
    ...
    info_lines = []
    if company:
        info_lines.append(f"회사: {company}")
    info_lines.append(f"기술: {tech_text}")
    info_lines.append(f"도메인: {domain_text}")
    info_text = "\n".join(info_lines)

    news_block = ""
    for i, n in enumerate(news, start=1):
        news_block += (
            f"[{i}] {n['title']} — {n['provider']} — {n['datePublished']}\n"
            f"요약: {n['snippet']}\nURL: {n['url']}\n\n"
        )
...
if click_search:
    ...
    if not q_now:
        st.warning("검색어를 입력하세요. (회사 선택 안 함 상태라면 기술/도메인만으로도 검색 가능합니다)")
    else:
        try:
            with st.spinner("한국어 뉴스 수집 중..."):
                results = fetch_news_ko(q_now, k, freshness, use_and=strict_and)
                st.session_state["news_results"] = results
            if results:
                st.success(f"뉴스 {len(results)}건 수집 완료 (한국어 전용)")
            else:
                st.warning("결과가 없습니다. 검색어를 단순화하거나 신선도를 Month로 넓혀보세요.")
...
st.divider()
st.caption("회사 없이도 기술·도메인만으로 검색/분석 가능합니다. 필요 시 '모든 키워드 반드시 포함(AND)' 옵션을 사용하세요.")
