# app.py
# --- Streamlit MVP: 한국어 뉴스 기반 PEST & SWOT (회사 선택 Selectbox 버전) ---
# 환경변수 (.env 예시)
#   NEWSAPI_KEY=...
...
# ✅ 회사 선택(1번 코딩 스타일) + 기타(직접입력)
with colA:
    company_choice = st.selectbox(
        "회사 선택",
        ["삼성SDS", "LG CNS", "SK C&C", "현대오토에버", "카카오엔터프라이즈", "네이버클라우드", "KT DS", "기타(직접입력)"],
        index=0,
    )
...
st.divider()
st.caption("한국어 뉴스만 사용합니다. 회사/기술/도메인 선택을 조합해 검색어가 자동 구성됩니다.")
